...
#se crea la funcion log para determinar el tiempo de ejecucion de cada paso
...
def transform():
    ...
    df_tracks = pd.read_sql_query("""SELECT t.TrackId, t.Name as TrackName, al.Title as Album, g.Name as Genre, mt.Name as MediaType, COALESCE(t.Composer, 'N/A') as Composer, t.Milliseconds, t.Bytes, t.UnitPrice as Price 
        ...
        ORDER BY 1;
        """, con=engine.connect())
    df_artists = pd.read_sql_query("""select ar.ArtistId, ar.Name as ArtistName 
        FROM artists ar;
        """, con=engine.connect())
